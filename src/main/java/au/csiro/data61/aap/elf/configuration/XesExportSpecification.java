...
public class XesExportSpecification extends InstructionSpecification<AddXesElementInstruction> {
    ...
    public static XesExportSpecification ofEventExport(ValueAccessorSpecification pid,
            ValueAccessorSpecification piid, ValueAccessorSpecification eid,
    ...
    public static XesExportSpecification ofEventExport(ValueAccessorSpecification pid,
        ...
        final AddXesEventInstruction instruction = new AddXesEventInstruction(
                ...
                eid == null ? null : eid.getValueAccessor(), parameters.stream()
                        .map(XesParameterSpecification::getParameter).collect(Collectors.toList()));
        return new XesExportSpecification(instruction);
    }

    public static XesExportSpecification ofTraceExport(ValueAccessorSpecification pid,
            ValueAccessorSpecification piid, XesParameterSpecification... parameters) {
        return ofTraceExport(pid, piid, Arrays.asList(parameters));
    }

    public static XesExportSpecification ofTraceExport(ValueAccessorSpecification pid,
            ValueAccessorSpecification piid, List<XesParameterSpecification> parameters) {
        assert parameters != null && parameters.stream().allMatch(Objects::nonNull);
        final AddXesTraceInstruction instruction = new AddXesTraceInstruction(
                pid == null ? null : pid.getValueAccessor(),
                piid == null ? null : piid.getValueAccessor(), parameters.stream()
                        .map(XesParameterSpecification::getParameter).collect(Collectors.toList()));
        return new XesExportSpecification(instruction);
    }
